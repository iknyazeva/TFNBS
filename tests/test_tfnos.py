...
from bnetdiff.pairwise_tfns import compute_t_stat
import time
...
class TestTFNOS(TestCase):
    ...
    def test_small_matrix(self):
        ...
        result_neg = get_tfce_score(-statsmat, 1, 1, 2)
    ...
    def test_small_matrix_edge_reduction(self):
        ...
        # Expected output (manually calculated)
        expected = np.array([[0.0, 3.0, 7.0], [3.0, 0.0, 3.0], [7.0, 3.0, 0.0]])
        np.testing.assert_allclose(result_edge, result, rtol=1e-5)
        np.testing.assert_allclose(result, expected, rtol=1e-5)
    ...
    def test_small_matrix_diff_pars(self):
        ...
        """
        statsmat = self.small_matrix
        result = get_tfce_score(statsmat, 0.4, 4, 100)

        # Expected output (manually calculated)
        expected = np.array([[0.0, 0.326, 8.706], [0.326, 0.0, 0.326], [8.706, 0.326, 0.0]])
